...
/atom/proc/visible_message(message, self_message, blind_message, vision_distance, ignored_mob)
	...
	for(var/mob/M in get_hearers_in_view(range, src))
		...
		if(M == src) //the src always see the main message or self message
			...
			else if(T.lighting_object || T.lighting_object)
				if(((T.lighting_object.invisibility <= M.see_invisible) || (T.lighting_object.invisibility <= M.see_invisible)) && T.is_softly_lit()) //the light object is dark and not invisible to us
					if(blind_message)
						msg = blind_message
					else
						continue
